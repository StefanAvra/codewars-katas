...
def string_color(name):
    ...
    first = sum(ascii_values) % 256
    product = numpy.prod(ascii_values) % 256
    last = abs(ascii_values[0] - sum(ascii_values[1:])) % 256
    return (format(first, 'x').zfill(2) + format(product, 'x').zfill(2) + format(last, 'x').zfill(2)).upper()
